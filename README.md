...
이러한 장점에 더불어 CloudFront는 S3와 함께 사용하여 정적 웹사이트 호스팅의 성능을 크게 향상시킬 수 있습니다



## 캐시 무효화

CloudFront에 캐시된 콘텐츠를 강제로 업데이트하는 프로세스입니다. 원본 콘텐츠가 변경되었을 때 캐시된 이전 버전을 제거하고 새로운 콘텐츠로 업데이트하기 위해 사용됩니다. 특정 파일 경로나 와일드카드 패턴을 지정하여 무효화할 수 있습니다. 주요 방법은 아래와 같습니다.
- Purge: 캐시된 콘텐츠를 즉시 제거
- Refresh: 애플리케이션에서 새로운 콘텐츠를 가져와 캐시 업데이트
- Ban: 캐시된 콘텐츠에 대한 참조를 블랙리스트에 추가

## Repository secret과 환경변수

- GitHub Actions에서 민감한 정보를 안전하게 저장하고 사용하기 위해 repository secret과 환경변수를 활용할 수 있습니다.
- Repository secrets: 저장소 수준에서 설정되며, 모든 워크플로우에서 접근 가능
- Environment secrets: 특정 환경에 대해 설정되며, 해당 환경을 사용하는 작업에서만 접근 가능
- Variables: 일반적인 설정 값을 저장하며, UI에서 값을 볼 수 있음
- Secrets는 로그나 UI에서 값이 노출되지 않지만, variables는 값이 표시됩니다. 환경을 사용하면 보호된 브랜치에서만 특정 secrets에 접근할 수 있도록 제한할 수 있습니다
